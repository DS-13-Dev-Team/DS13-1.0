/turf/simulated/floor/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(flooring)
		if(isCrowbar(C))
			...
			return
		else if(istype(C, /obj/item/weapon/tool/shovel) && (flooring.flags & TURF_REMOVE_SHOVEL))
		...
		// Repairs and Deconstruction.
			...
			if((is_plating()) && C.use_tool(user, src, WORKTIME_SLOW, QUALITY_WELDING, FAILCHANCE_NORMAL))
				if(broken || burnt)
