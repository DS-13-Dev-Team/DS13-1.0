...
/obj/machinery/telecomms/New(var/atom/location, var/direction, var/nocircuit = FALSE)
	telecomms_list += src
	..()
...
/obj/machinery/telecomms/relay/forceMove(newloc, glide_size_override)
	. = ..(newloc)
...
/obj/machinery/telecomms/server/New(var/atom/location, var/direction, var/nocircuit = FALSE)
	..()
	Compiler = new()
	Compiler.Holder = src
	server_radio = new()
	...
	if(signal.data["message"])
		...
		if(is_freq_listening(signal))
			...
			if(signal.data["type"] != 4)
				...
				if(!istype(M, /mob/dead/new_player) && M)
