...
/obj/machinery/recycling_tank/proc/add_purified_biomass(quantity)
	if (!storage)
		return FALSE
...
/obj/machinery/recycling_tank/proc/is_valid(atom/test, mob/user)
	if (test.get_biomass())
		return TRUE
	else
		if (user)
...
/obj/machinery/recycling_tank/proc/try_insert_atom(atom/A, mob/user)
	if(!is_valid(A, user))
		return
