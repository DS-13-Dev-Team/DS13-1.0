...
/obj/machinery/door/firedoor
	...
	blend_objects = list(/obj/machinery/door/firedoor, /obj/structure/wall_frame, /turf/unsimulated/wall, /obj/structure/window, /obj/structure/tramwall) // Objects which to blend with
...
/obj/machinery/door/firedoor/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(isWelder(C) && !repairing)
		if(C.use_tool(user, src, WORKTIME_SLOW, QUALITY_WELDING, FAILCHANCE_NORMAL))
			blocked = !blocked
			user.visible_message("<span class='danger'>\The [user] [blocked ? "welds" : "unwelds"] \the [src] with \a [C].</span>",\
			"You [blocked ? "weld" : "unweld"] \the [src] with \the [C].",\
			...
			for(var/d=1;d<=4;d++)
				...
				for(var/i=1;i<=ALERT_STATES.len;i++)
					if(dir_alerts[d] & (1<<(i-1)))
						overlays += icon(icon,"alert_[ALERT_STATES[i]]", dir=cdir)
						do_set_light = TRUE
	else
		icon_state = "open"
