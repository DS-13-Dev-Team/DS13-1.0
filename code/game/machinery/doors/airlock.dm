...
/obj/machinery/door/airlock
	...
	var/obj/item/airlock_brace/brace
	...
	switch(animation)
		...
		if("deny")
			if(density && src.arePowerSystemsOn())
				...
				flick("deny", src)
				playsound(src.loc, open_failure_access_denied, VOLUME_MID, 0) //secured wires condition delete so we can actually hear noise
				update_icon(AIRLOCK_CLOSED)
		if("emag")
			if(density && src.arePowerSystemsOn())
...
/obj/machinery/door/airlock/attackby(var/obj/item/C, var/mob/user)
	if (!ismob(C))
		...
		if(brace && (istype(C.GetIdCard(), /obj/item/card/id/) || istype(C, /obj/item/tool/crowbar/brace_jack)))
		...
		else if (istype(C, /obj/item/material/twohanded/fireaxe) && !(stat & BROKEN) && (src.health <= src.max_health / 2) && user.a_intent == I_HURT)
...
/obj/machinery/door/airlock/open(var/forced=0)
	...
	if(arePowerSystemsOn())
		playsound(src.loc, open_sound_powered, VOLUME_LOW, 0)
	else
		playsound(src.loc, open_sound_unpowered, VOLUME_LOW, 0)
...
/obj/machinery/door/airlock/close(var/forced=0)
	...
	use_power(360)	//360 W seems much more appropriate for an actuator moving an industrial door capable of crushing people
	if(arePowerSystemsOn())
		playsound(src.loc, close_sound_powered, VOLUME_LOW, 0)
	else
		playsound(src.loc, close_sound_unpowered, VOLUME_LOW, 0)
...
/obj/machinery/door/airlock/proc/try_reinforce(obj/item/airlock_brace/B, mob/M)
	if(!density)
