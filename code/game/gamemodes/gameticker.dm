...
//		emergency_shuttle.process() //handled in scheduler
		...
		var/game_finished = 0
		var/mode_finish_result = mode.check_finished()
		if (mode_finish_result = GAME_MAYBE_FINISHED)
			mode.maybe_finished()
		var/mode_finished = 0

		if (CONFIG_GET(flag/continous_rounds))
			game_finished = (evacuation_controller.round_over() || mode.station_was_nuked)
			mode_finished = (!post_game && (mode_finish_result == GAME_FINISHED))
		else
			game_finished = ((mode_finish_result == GAME_FINISHED) || (evacuation_controller.round_over() && evacuation_controller.emergency_evacuation) || universe_has_ended)
