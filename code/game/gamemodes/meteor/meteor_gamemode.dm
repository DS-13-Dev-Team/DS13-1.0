...
/datum/game_mode/meteor/process()
	...
	if((gameTime() >= next_wave) && (alert_sent == 1))
		...
		next_wave = gameTime() + (meteor_wave_delay * time_between_waves_minutes)
	if((gameTime() >= METEOR_FAILSAFE_THRESHOLD) && (meteor_severity < 15) && !failsafe_triggered)
		log_and_message_admins("Meteor mode severity failsafe triggered: Severity forced to 15.")
		meteor_severity = 15
		failsafe_triggered = 1

	if(gameTime() >= next_wave)
		next_wave = gameTime() + (meteor_wave_delay * time_between_waves_minutes)
