...
/datum/antagonist/earthgov_agent/proc/give_collaborators(mob/living/our_owner)
	...
	for(var/mob/living/minion in GLOB.agent_list)
...
/datum/codex_entry/concept/earthgov_agents
	...
	mechanics_text = "For the purpose of this game setting, no one is specifically associated with EarthGov unless:<br>\
	* They are an EarthGov agent sent specifically with a task, or have been coopted into an EarthGov operation.<br>\
	* Are a marine within the Earth Defense Force.<br><br>\
	\
