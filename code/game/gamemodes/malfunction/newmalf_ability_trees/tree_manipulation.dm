...
/datum/game_mode/malfunction/verb/reboot_camera(var/obj/machinery/camera/target in GLOB.cameranet.cameras)
...
/datum/game_mode/malfunction/verb/machine_overload(obj/machinery/M in SSmachines.machinery)
	...
	spawn(5 SECONDS)
		M.explosion(round(explosion_intensity),round(explosion_intensity/2))
		if(M)
			qdel(M)
