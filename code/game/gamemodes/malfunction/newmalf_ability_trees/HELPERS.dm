...
/datum/game_mode/malfunction/verb/ai_select_hardware()
	...
	if(!note)
		log_debug("Hardware without description: [C]")
		return

	var/confirmation = tgui_alert(usr, "[note] - Is this what you want?", "Hardware selection", list("Yes", "No"))
	if(confirmation != "Yes")
...
/datum/game_mode/malfunction/verb/ai_help()
	...
	var/help = file2text('html/browser/malf_ai.html')
	if(!help)
		help = "Error loading help (file /html/browser/malf_ai.html is probably missing). Please report this to server administration staff."
