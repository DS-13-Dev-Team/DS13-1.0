...
/datum/uplink_random_item/New(var/uplink_item, keep_probability = 100, reselect_propbability = 33)
	..()
...
/datum/uplink_random_selection/blacklist/get_random_item(var/telecrystals, obj/item/device/uplink/U, list/bought_items)
	var/const/attempts = 50
	for(var/i = 0; i < attempts; i++)
		var/datum/uplink_random_item/RI = pick(items)
		if(!prob(RI.keep_probability))
