...
/obj/item/weapon/tool_modification/reinforcement/heatsink/can_apply(obj/item/weapon/tool/T, mob/user)
	.=..()
	if (.)
...
/obj/item/weapon/tool_modification/productivity/diamond_blade/can_apply(obj/item/weapon/tool/T, mob/user)
	.=..()
	if (.)
...
/obj/item/weapon/tool_modification/productivity/motor/can_apply(obj/item/weapon/tool/T, mob/user)
	.=..()
	if (.)
		if (T.use_fuel_cost || T.use_power_cost)
			return TRUE
...
/obj/item/weapon/tool_modification/augment/cell_mount/can_apply(obj/item/weapon/tool/T, mob/user)
	.=..()
	if (.)
		if (T.suitable_cell == /obj/item/weapon/cell || T.suitable_cell == /obj/item/weapon/cell)
			if (T.cell)
				user << SPAN_DANGER("You'll need to remove the power cell before installing this modification. It won't be compatible afterwards")
				return FALSE
			return TRUE
		else
			return FALSE
