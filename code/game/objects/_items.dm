...
/obj/item/proc/openTip(location, control, params, user)
	if(last_force_string_check != force && !(item_flags & OBJ_FLAG_FORCE_STRING_OVERRIDE))
		set_force_string()
	if(!(item_flags & OBJ_FLAG_FORCE_STRING_OVERRIDE))
...
/obj/item/proc/apply_outline(outline_color = null)
	...
	if(!outline_color) //if we weren't provided with a color, take the theme's color
		switch(theme) //yeah it kinda has to be this way
			...
			if("Orange")
				outline_color = COLOR_THEME_ORANGE
			if("old")
				outline_color = COLOR_THEME_OLD //just as garish as the rest of this theme
