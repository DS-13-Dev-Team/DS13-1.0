...
/mob/proc/lurch(direction, camera_shift = 64, skip_cooldown = TRUE)
	if (!direction)
...
*/
/mob/proc/resurrect(external_healing = 100)
	if (stat != DEAD)
		return FALSE
...
/mob/living/resurrect(external_healing = 100)
	.=..()
	if (!.)
		return
	if (external_healing)
...
/mob/living/carbon/human/resurrect(external_healing = 100)
	.=..()
	if (!.)
		return
