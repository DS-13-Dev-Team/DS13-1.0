...
#define ZONE_MIN_SIZE 6 //zones with less than this many turfs will always merge, even if the connection is not direct
...
#define ATMOS_CANPASS_TURF(ret,A,B) \
	if (A.blocks_air & AIR_BLOCKED || B.blocks_air & AIR_BLOCKED) { \
		...
		if (B.z < A.z) { \
			ret = (A.turf_flags & TURF_PASS_ATMOS_Z) ? ZONE_BLOCKED : BLOCKED; \
		} \
		else { \
			ret = (B.turf_flags & TURF_PASS_ATMOS_Z) ? ZONE_BLOCKED : BLOCKED; \
		} \
	} \
