#define CLIENT_MIN_FPS 0
#define CLIENT_MAX_FPS 1000

//Key modifier defines that can be used for click verbs
#define	KEY_ALT		/datum/click_handler/modifier/alt
#define	KEY_CTRL	/datum/click_handler/modifier/ctrl
#define KEY_SHIFT	/datum/click_handler/modifier/shift
#define KEY_MIDDLE	/datum/click_handler/modifier/middle
#define KEY_CTRLALT	/datum/click_handler/modifier/ctrlalt
#define KEY_CTRLSHIFT	/datum/click_handler/modifier/ctrlshift

#define SACCADE_BASE_SPEED	0.45	//Time in deciseconds to move the camera over one tile

//This exists just to save on proc-call overhead
#define RESOLVE_CLICK_CATCHER(params, C)	get_turf_at_mouse(params,C);