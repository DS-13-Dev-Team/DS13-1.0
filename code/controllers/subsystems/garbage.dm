...
/datum/controller/subsystem/garbage/proc/HardDelete(datum/D, actually_delete_it)
...
/datum/proc/DoSearchVar(X, Xname, recursive_limit = 64)
	...
	if(istype(X, /datum))
		...
		for(var/varname in L)
			...
			if(variable == src)
#ifdef TESTING
				var/message = "Found [src.type] [any2ref(src)] in [D.type] [varname] var. [Xname]"
				testing(message)
#endif
		...
		for(var/I in X)
			if (I == src)
#ifdef TESTING
				var/message = "Found [src.type] [any2ref(src)] in list [Xname]."
				testing(message)
#endif

			else if (I && !isnum(I) && normal && X[I] == src)
#ifdef TESTING
				var/message = "Found [src.type] [any2ref(src)] in list [Xname]\[[I]\]"
				testing(message)
#endif
