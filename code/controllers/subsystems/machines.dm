...
/datum/controller/subsystem/machines/stat_entry(msg)
	msg = "|Q: P [length(pipenets)] M [length(processing)] N [length(powernets)] O [length(power_objects)] |C: P [round(cost_pipenets)]	M [round(cost_machinery)] N [round(cost_powernets)] O [round(cost_power_objects)] |Overall [round(cost ? processing.len / cost : 0, 0.1)]"
	return ..()
