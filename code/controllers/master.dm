...
/datum/controller/master/Initialize(delay, init_sss, tgs_prime)
	...
	for (var/datum/controller/subsystem/subsystem as anything in subsystems)
		...
		if (!isnum(subsystem_init_stage) || subsystem_init_stage < 1 || subsystem_init_stage > INITSTAGE_MAX || round(subsystem_init_stage) != subsystem_init_stage)
			crash_with("ERROR: MC: subsystem `[subsystem.type]` has invalid init_stage: `[subsystem_init_stage]`. Setting to `[INITSTAGE_MAX]`")
...
/datum/controller/master/proc/Loop(init_stage)
	...
	while (1)
		...
		if (CheckQueue(subsystems_to_check) <= 0) //error processing queue
			crash_with("MC: CheckQueue failed. Current error_level is [round(error_level, 0.25)]")
		...
		if (queue_head)
			if (RunQueue() <= 0) //error running queue
				crash_with("MC: RunQueue failed. Current error_level is [round(error_level, 0.25)]")
...
/datum/controller/master/proc/SoftReset(list/ticker_SS, list/runlevel_SS)
	. = 0
	crash_with("MC: SoftReset called, resetting MC queue state.")
