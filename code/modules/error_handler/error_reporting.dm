...
/hook/roundend/proc/send_runtimes_to_ircbot()
	if(!GLOB.revdata.originmastercommit) return // we can't do much useful if we don't know what we are
	...
	runtimes2irc(list2params(errors), GLOB.revdata.get_log_message())
