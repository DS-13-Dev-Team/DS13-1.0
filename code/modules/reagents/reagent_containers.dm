...
/obj/item/reagent_containers/proc/standard_pour_into(var/mob/user, var/atom/target, var/quantity_override = null) // This goes into afterattack and yes, it's atom-level
	if(!target.reagents)
		return 0
...
/obj/item/reagent_containers/do_surgery(mob/living/carbon/M, mob/living/user)
	if(get_zone_sel(user, TRUE) != BP_MOUTH) //in case it is ever used as a surgery tool
		return ..()
...
/obj/item/reagent_containers/adjust_biomass(var/change)
...
/obj/item/reagent_containers/get_biomass()
	if (isnull(biomass))
...
/obj/item/reagent_containers/calculate_biomass()
