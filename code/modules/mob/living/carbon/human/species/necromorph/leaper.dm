...
/datum/species/necromorph/leaper/monkey
	name = SPECIES_NECROMORPH_LEAPER_MONKEY
	marker_spawnable = FALSE 	//Enable this once we have sprites for it
	mob_type	=	/mob/living/carbon/human/necromorph/leaper/monkey
...
/datum/unarmed_attack/claws/leaper/monkey
	damage = 6.5
...
/mob/living/carbon/human/proc/tailstrike_leaper_monkey(var/atom/A)
	...
	//The sound has a randomised delay
	if(tailstrike_attack(A, _damage = 14, _windup_time = 0.6 SECONDS, _winddown_time = 0.9 SECONDS, _cooldown = 0))
		spawn(rand_between(0, 1.8 SECONDS))
			play_species_audio(src, SOUND_ATTACK, 30, 1)
...
/mob/living/proc/leaper_gallop_monkey()
	...
	if (gallop_ability(_duration = 2 SECONDS, _cooldown = 8 SECONDS, _power = 1))
		H.play_species_audio(H, SOUND_SHOUT, VOLUME_MID, 1, 3)
