...
/mob/living/bot/mulebot
	...
	target_speed = 3
	max_frustration = 5
	botcard_access = list(access_maint_tunnels, access_cargo, access_mining)
		...
		switch(command)
			...
			if("sethome")
				...
				if(beaconlist.len)
					...
					tgui_alert(user, "No destination beacons available.")
				if(new_dest)
	...
	switch(command)
		...
		if("SetD")
			...
			if(beaconlist.len)
				...
				tgui_alert(usr, "No destination beacons available.")
			if(new_dest)
				resetTarget()
