...
/proc/hearers_in_range(var/atom/movable/center_vmob, var/hearing_range = world.view)
	...
	ACQUIRE_VIRTUAL_OR_TURF(center_vmob)
	for(var/mob/dead/observer/virtual/v_mob in range(hearing_range, center_vmob))
...
/proc/all_hearers(var/atom/movable/heard_vmob, var/range = world.view)
	...
	for(var/mob/dead/observer/virtual/v_mob in hearers(range, heard_vmob))
		if(v_mob.abilities & VIRTUAL_ABILITY_HEAR)
			. |= v_mob.host
...
/proc/all_viewers(var/mob/dead/observer/virtual/viewed_atom)
	. = list()
...
/mob/dead/observer/eye/virtual_can_see_turf(var/turf/T)
