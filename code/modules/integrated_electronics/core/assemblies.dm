...
/obj/item/device/electronic_assembly/attackby(obj/item/I, mob/living/user)
	if(istype(I, /obj/item/weapon/tool/wrench))
		...
		if(try_add_component(I, user))
			...
			return ..()
	else if(istype(I, /obj/item/weapon/tool/multitool) || istype(I, /obj/item/device/integrated_electronics/wirer) || istype(I, /obj/item/device/integrated_electronics/debugger))
		if(opened)
			interact(user)
			return TRUE
		else
