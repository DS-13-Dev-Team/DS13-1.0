...
/datum/custom_item/proc/apply_to_item(var/obj/item/item)
	...
	if(istype(item, /obj/item/device/kit))
		...
		if(istype(item, /obj/item/device/kit/paint))
			...
			kit.allowed_types = splittext_char(additional_data, ", ")
...
/hook/startup/proc/load_custom_items()
	...
	for(var/line in splittext_char(file2text("config/custom_items.txt"), "\n"))
		...
		switch(field)
			...
			if("req_titles")
				current_data.req_titles = splittext_char(field_data,", ")
			if("kit_name")
