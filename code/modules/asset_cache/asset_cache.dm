...
/proc/register_asset(asset_name, asset)
	...
	if (SSassets.cache[asset_name])
		...
		if (OACI.hash != ACI.hash)
			crash_with("ERROR: new asset added to the asset cache with the same name as another asset: [asset_name] existing asset md5: [OACI.hash] new asset md5:[ACI.hash]")
	SSassets.cache[asset_name] = ACI
	return ACI

/// Returns the name of the asset, currently this is just its name, here to allow further work cdn'ing assets.
