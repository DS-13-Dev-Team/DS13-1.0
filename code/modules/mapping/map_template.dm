/datum/map_template
	...
	var/turf/ceiling_turf = /turf/simulated/floor/plating
...
/datum/map_template/proc/initTemplateBounds(list/bounds)
	if (!bounds) //something went wrong
		crash_with("[name] template failed to initialize correctly!")
		return
...
/datum/map_template/proc/load_new_z(secret = FALSE)
	...
	var/datum/parsed_map/parsed = load_map(file(mappath), x, y, level.z_value, no_changeturf=(SSatoms.initialized == INITIALIZATION_INSSATOMS))
...
/datum/map_template/proc/generate_ceiling(affected_turfs)
	for (var/turf/turf in affected_turfs)
		...
		if (ceiling)
			if (istype(ceiling, /turf/simulated/open))
				ceiling.ChangeTurf(ceiling_turf)
