...
/client/proc/create_dungeon()
	...
	if(tgui_alert(src, "Would you like to modify generated room/corridor variables?",, list("No","Yes")) == "Yes")
	...
	if(tgui_alert(src, "Would you like to modify generational multipliers?",,list("No","Yes")) == "Yes")
	...
	while(1)
		...
		switch(mode)
			if("room_theme")
				if(tgui_alert(src,"Would you like to modify what room layouts are used?",,list("No","Yes")) != "Yes")
					mode = "monsters"
					continue
			if("monsters")
				if(tgui_alert(src,"Would you like to modify what mobs get spawned?",,list("No","Yes")) != "Yes")
					mode = "loot"
					continue
			if("loot")
				if(tgui_alert(src,"Would you like to modify what loot gets spawned?",,list("No","Yes")) !="Yes")
					break
		...
		while(1)
			...
			if(!layout)
				if(current_list == common && tgui_alert(src,"Would you like to add items to the uncommon pool?",,list("No","Yes")) == "Yes")
					current_list = uncommon
					continue
				else if(current_list != rare && tgui_alert(src,"Would you like to add items to the rare pool?",, list("No","Yes")) == "Yes")
					current_list = rare
					continue
				else
					break
			var/chance = input("What is the weight of this item? Higher weights get picked more frequently. Default is 10",10) as num
			if(tgui_alert(src,"Do you want to add all subtypes of this item to the current pool as well? They will be weighted the same.",,list("No","Yes")) == "Yes")
				var/list/local_total = typesof(layout)
				if(tgui_alert(src,"Do you want to only add the subtypes of this item?",,list("No","Yes")) == "Yes")
					local_total -= layout
	...
	if(tgui_alert(src,"Are you sure you want to create this? It will appear at your position.",,list("No","Yes")) != "Yes")
		return
