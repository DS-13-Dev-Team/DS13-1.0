...
/datum/random_map/droppod/apply_to_map()
	if(placement_explosion_dev || placement_explosion_heavy || placement_explosion_light || placement_explosion_flash)
		...
		if(istype(T))
			T.explosion(placement_explosion_dev, placement_explosion_heavy)
...
/datum/random_map/droppod/proc/get_spawned_drop(var/turf/T)
	...
	C.set_light_range(3)
	C.set_light_power(0.5)
	C.set_light_color("#cc0000")
	var/mob/living/drop
...
/datum/admins/proc/call_drop_pod()
	...
	if(tgui_alert(usr, "Do you wish the mob to have a player?", "Mob Mind", list("No","Yes")) != "Yes")
	...
	if(tgui_alert(usr, "Are you SURE you wish to deploy this drop pod? It will cause a sizable explosion and gib anyone underneath it.", "Confirmation", list("No","Yes")) != "Yes")
		if(spawned_mob)
			qdel(spawned_mob)
		if(spawned_mobs.len)
