...
/datum/random_map/droppod/supply/get_spawned_drop(var/turf/T)
	...
	if(istype(drop_type, /datum/supply_drop_loot))
		...
		log_debug("Unhandled drop type: [drop_type]")
		...
		if(choice == "Yes")
			while(1)
				...
				if(!adding_loot_type)
					break
				chosen_loot_types |= adding_loot_type
		choice = tgui_alert(usr, "Do you wish to add ABSOLUTELY ANYTHING ELSE? (Fast way to crash your Dream Seeker)",, list("No","Yes"))
		if(choice == "Yes")
			while(1)
