...
/datum/proc/ui_modal_data(datum/source = src)
	...
	var/datum/ui_modal/current = LAZYACCESS(GLOB.tgui_modals, REF(source))
	if(!current)
		return null
...
/datum/proc/ui_modal_clear(datum/source = src)
	...
	var/datum/ui_modal/previous = GLOB.tgui_modals[REF(source)]
	if(!previous)
		return FALSE
...
/**
  ...
  */
/datum/proc/ui_modal_input(datum/source = src, id, text = "Default modal message", delegate, arguments, value = "", max_length = TGUI_MODAL_INPUT_MAX_LENGTH)
	ASSERT(length(id))
	ASSERT(max_length > 0)
...
/datum/proc/ui_modal_new(datum/source = src, datum/ui_modal/modal = null, replace_previous = TRUE, instant_update = TRUE)
	...
	var/datum/ui_modal/previous = LAZYACCESS(GLOB.tgui_modals, REF(source))
