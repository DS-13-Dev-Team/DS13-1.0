...
/obj/item/modular_computer/proc/install_default_programs_by_job(var/mob/living/carbon/human/H)
	var/datum/job/jb = job_master.GetJob(H.job)
	if(!jb) return
...
/obj/item/modular_computer/proc/kill_program(var/forced = 0)
	...
	if(user && istype(user))
		...
		INVOKE_ASYNC(src, /datum/proc/ui_interact, usr) // Re-open the UI on this computer. It should show the main screen now.
	update_icon()
