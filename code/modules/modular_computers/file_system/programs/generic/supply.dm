...
/datum/nano_module/supply/Topic(href, href_list)
	...
	if(href_list["order"])
		...
		if(can_print() && tgui_alert(user, "Would you like to print a confirmation receipt?", "Print receipt?", list("Yes", "No")) == "Yes")
			print_order(O, user)
		return 1
...
/datum/nano_module/supply/proc/print_summary(var/mob/user)
	var/t = ""
	t += "<center><BR><b><large>[GLOB.using_map.station_name]</large></b><BR><i>[GLOB.current_date_string]</i><BR><i>Export overview<field></i></center><hr>"
