...
/obj/item/clothing/mask/monitor/mob_can_equip(var/mob/M, var/slot, var/disable_warning = 0, var/force = 0)
	if (!..())
		return 0
	if(ishuman(M))
		...
		if(istype(E) && BP_IS_ROBOTIC(E))
			return 1
		to_chat(M, "<span class='warning'>You must have a robotic head to install this upgrade.</span>")
	return 0
