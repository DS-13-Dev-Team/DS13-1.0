...
/proc/log_and_message_admins(var/message as text, mob/user = usr, turf/location)
...
/proc/log_and_message_staff(var/message as text, mob/user = usr, turf/location)
	var/turf/T = location ? location : (user ? get_turf(user) : null)
	message = append_admin_tools(message, user, T)
...
/proc/log_and_message_admins_many(var/list/mob/users, message)
	if(!users || !users.len)
		return
...
/proc/admin_attacker_log(var/mob/attacker, attacker_message)
	if(!attacker)
		EXCEPTION("No attacker was supplied.")
	admin_attack_log(attacker, null, attacker_message, null, attacker_message)

/proc/admin_victim_log(var/mob/victim, victim_message)
	if(!victim)
		EXCEPTION("No victim was supplied.")
	admin_attack_log(null, victim, null, victim_message, victim_message)

/proc/admin_attack_log(var/mob/attacker, mob/victim, attacker_message, victim_message, admin_message)
	if(!(attacker || victim))
...
/proc/store_admin_attack_log(var/mob/attacker, mob/victim)
...
/proc/notify_about_admin_attack_log(var/mob/attacker, mob/victim)
	if(attacker && victim)
...
/proc/admin_attacker_log_many_victims(var/mob/attacker, list/mob/victims, attacker_message, victim_message, admin_message)
...
/proc/append_admin_tools(var/message, mob, turf/location)
	if(location)
