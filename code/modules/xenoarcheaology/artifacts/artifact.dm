...
/obj/machinery/artifact/New(var/atom/location, var/direction, var/nocircuit = FALSE)
	...
	if(icon_num == 7 || icon_num == 8)
		...
		if(prob(50))
			my_effect.trigger = pick(TRIGGER_ENERGY, TRIGGER_OXY, TRIGGER_NITRO)
	else if(icon_num == 11)
	...
	if (istype(W, /obj/item/reagent_containers/))
		if(W.reagents.has_reagent(/datum/reagent/hydrazine, 1) || W.reagents.has_reagent(/datum/reagent/water, 1))
			...
			if(secondary_effect && secondary_effect.trigger == TRIGGER_WATER && prob(25))
				secondary_effect.ToggleActivate(0)
		else if(W.reagents.has_reagent(/datum/reagent/acid, 1) || W.reagents.has_reagent(/datum/reagent/acid/triflicacid, 1) || W.reagents.has_reagent(/datum/reagent/diethylamine, 1))
