...
/obj/machinery/atmospherics/omni/filter/proc/mode_send_switch(var/mode = ATM_NONE)
	switch(mode)
		...
		if(ATM_P)
			return MATERIAL_PHORON //*cough* Plasma *cough*
		if(ATM_N2O)
...
/obj/machinery/atmospherics/omni/filter/Topic(href, href_list)
	...
	if(configuring && !use_power)
		switch(href_list["command"])
			...
			if("switch_filter")
				var/new_filter = input(usr,"Select filter mode:","Change filter",href_list["mode"]) in list("None", "Oxygen", "Nitrogen", "Carbon Dioxide", MATERIAL_PHORON, "Nitrous Oxide", "Hydrogen")
...
/obj/machinery/atmospherics/omni/filter/proc/switch_filter(var/dir, mode)
...
/obj/machinery/atmospherics/omni/filter/proc/switch_mode(var/port, mode)
	if(mode == null || !port)
		return
