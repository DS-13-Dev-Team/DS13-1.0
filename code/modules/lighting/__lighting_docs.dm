...
atom/movable/lighting_overlay: (lighting_overlay.dm)
  - var/lum_r, lum_g, lum_b; lumcounts of each colour
  - var/needs_update; set on update_lumcount, checked by lighting process

  - var/xoffset, yoffset; (only present when using sub-tile overlays) fractional offset of this overlay in the tile
