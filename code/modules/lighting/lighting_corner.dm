...
/datum/lighting_corner/proc/update_lumcount(delta_r, delta_g, delta_b, now = FALSE)
	...
	if (t1 && (T = GetAbove(t1)) && (isopenspace(T)))
		Ti = t1i
	else if (t2 && (T = GetAbove(t2)) && (isopenspace(T)))
		Ti = t2i
	else if (t3 && (T = GetAbove(t3)) && (isopenspace(T)))
		Ti = t3i
	else if (t4 && (T = GetAbove(t4)) && (isopenspace(T)))
		Ti = t4i
...
/datum/lighting_corner/Destroy(force = FALSE)
	crash_with("Someone [force ? "force-" : ""]deleted a lighting corner.")
	if (!force)
