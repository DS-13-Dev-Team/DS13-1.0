...
/obj/machinery/elevator_control_panel/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	playsound(src, "sparks", 100, TRUE, -9)
	to_chat(user, SPAN_NOTICE("You successfully override safety."))
	emagged = TRUE

/obj/machinery/elevator_control_panel/attackby(obj/item/O, mob/user)
	if(isMultitool(O))
		var/datum/lift_master/lift = lift_weakref?.resolve()
		if(!lift)
			return
		...
		to_chat(user, SPAN_NOTICE("Panel was succesfully reset."))
		playsound(src, 'sound/machines/locktoggle.ogg', 50, TRUE)
...
/obj/machinery/elevator_control_panel/proc/add_destinations_in_a_direction_recursively(list/turfs_to_check, direction, list/destinations)
	...
	for(var/turf/place in turfs_to_check)
		...
		if(direction == DOWN && !istype(place, /turf/simulated/open))
			return
		...
		if(direction == UP && !istype(next_level, /turf/simulated/open))
			return
...
/obj/machinery/elevator_control_panel/ui_data(mob/user)
	...
	data["emergency_level"] = capitalize(security_state.current_security_level.name)
	data["is_emergency"] = (istype(security_state.current_security_level, /decl/security_level/default/code_red) || istype(security_state.current_security_level, /decl/security_level/default/code_delta))
...
/obj/machinery/elevator_control_panel/ui_act(action, list/params)
	...
	switch(action)
		...
		if("emergency_door")
			...
			var/decl/security_state/security_state = decls_repository.get_decl(GLOB.using_map.security_state)
			if(!istype(security_state.current_security_level, /decl/security_level/default/code_red) && !istype(security_state.current_security_level, /decl/security_level/default/code_delta))
