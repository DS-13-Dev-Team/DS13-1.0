...
/datum/lift_master/proc/add_lift_platforms(obj/structure/industrial_lift/new_lift_platform)
	...
	for(var/obj/structure/industrial_lift/other_platform in new_lift_platform.loc)
		if(other_platform != new_lift_platform)
			crash_with("there is more than one lift platform on a tile when a lift_master adds it. this causes problems")
			qdel(other_platform)
...
/datum/lift_master/proc/create_multitile_platform_for_z_level(list/obj/structure/industrial_lift/platforms_in_z)
	...
	for(var/obj/structure/industrial_lift/lift_to_sort as anything in platforms_in_z)
		if(!z)
			if(!lift_to_sort.z)
				crash_with("create_multitile_platform_for_z_level() was given a platform in nullspace or not on a turf!")
		...
		if(z != lift_to_sort.z)
			crash_with("create_multitile_platform_for_z_level() was given lifts on different z levels!")
			platforms_in_z -= lift_to_sort
			continue
...
/datum/lift_master/proc/move_after_delay(lift_move_duration, door_duration, direction, mob/user)
	...
	for(var/turf/moving in lift_locs)
		...
		new /obj/effect/telegraphing/lift_travel(below_us, lift_move_duration)
...
/datum/lift_master/proc/update_lift_doors(on_z_level, action)
	...
	for(var/obj/machinery/door/airlock/multi_tile/civilian/tram/elevator_door in SSmachines.machinery)
		...
		switch(action)
			if(OPEN_DOORS)
				INVOKE_ASYNC(elevator_door, /obj/machinery/door/airlock/multi_tile/civilian/tram.proc/open)

			if(CLOSE_DOORS)
				INVOKE_ASYNC(elevator_door, /obj/machinery/door/airlock/multi_tile/civilian/tram.proc/close)

			else
				crash_with("Elevator lift update_lift_doors called with an improper action ([action]).")
...
/obj/effect/telegraphing/Initialize()
	QDEL_IN(src, duration)
	return ..()

/obj/effect/telegraphing/vending_machine_tilt
	duration = 1 SECONDS

/obj/effect/telegraphing/lift_travel

/obj/effect/telegraphing/lift_travel/Initialize(mapload, duration)
	src.duration = duration
	return ..()

/obj/effect/telegraphing/thunderbolt
