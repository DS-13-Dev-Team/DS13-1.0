...
/obj/structure/industrial_lift/proc/create_multitile_platform(min_x, min_y, max_x, max_y, z)
	...
	if(!loc_corner_lift)
		crash_with("no lift in the bottom left corner of a lift level!")
		return FALSE
...
/obj/structure/industrial_lift/proc/travel(going)
	...
	if(going == DOWN)
		for(var/turf/dest_turf as anything in entering_locs)
			...
			if(istype(dest_turf, /turf/simulated/wall))
				var/turf/simulated/wall/hit_wall = dest_turf
			...
			for(var/mob/living/crushed in dest_turf.contents)
				...
				if(violent_landing)
					...
					crushed.Stun(15)
					crushed.apply_damage(30, BRUTE, BP_CHEST)
					crushed.apply_damage(20, BRUTE, BP_HEAD)
					crushed.apply_damage(15, BRUTE, BP_L_LEG)
					crushed.apply_damage(15, BRUTE, BP_R_LEG)
					crushed.apply_damage(15, BRUTE, BP_L_ARM)
					crushed.apply_damage(15, BRUTE, BP_R_ARM)
		...
		for(var/turf/dest_turf as anything in entering_locs)
			...
			if(istype(dest_turf, /turf/simulated/wall))
				var/turf/simulated/wall/collided_wall = dest_turf
			...
			for(var/obj/structure/victim_structure in dest_turf.contents)
				...
				if(!is_type_in_typecache(victim_structure, lift_master_datum.ignored_smashthroughs) && victim_structure.layer >= LOW_OBJ_LAYER)
					...
					if(victim_structure.anchored && initial(victim_structure.anchored) == TRUE)
						visible_message(SPAN_USERDANGER("[src] smashes through [victim_structure]!"))
						qdel(victim_structure)

					else
						if(!throw_target)
							throw_target = get_edge_target_turf(src, turn(going, pick(45, -45)))
						visible_message(SPAN_USERDANGER("[src] violently rams [victim_structure] out of the way!"))
			...
			for(var/mob/living/collided in dest_turf.contents)
				...
				new /obj/effect/decal/cleanable/blood/splatter(get_turf(collided))
...
/obj/structure/industrial_lift/proc/group_move(list/atom/movable/movers, movement_direction)
	if(movement_direction == NONE)
		crash_with("an industrial lift was told to move to somewhere it already is!")
		return FALSE
...
/obj/structure/industrial_lift/proc/open_lift_radial(mob/living/user)
	...
	if(!length(possible_directions))
		to_chat(user, SPAN_WARNING("Elevator is out of service!"))
		return
	...
	if(!isnull(result) && result != "Cancel" && lift_master_datum.controls_locked)
		...
		to_chat(user, SPAN_WARNING("Elevator controls are locked!"))
		return
	switch(result)
		if("Up")
