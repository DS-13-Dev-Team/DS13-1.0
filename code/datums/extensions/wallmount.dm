...
*/
/proc/mount_to_atom(atom/movable/subject, atom/mountpoint, mount_type = /datum/extension/mount, datum/mount_parameters/WP = new())
...
/proc/is_valid_mount_target(atom/movable/AM, datum/mount_parameters/WP = new())
	.=FALSE
	if (QDELETED(AM))
		return
	if (isturf(AM))
...
/datum/extension/mount/proc/on_mountpoint_move(atom/mover, oldloc, newloc)
	if (mounted)
		ignore_mountee_move = TRUE
		mountee.forceMove(locate(mountpoint.x + offset.x, mountpoint.y + offset.y, mountpoint.z))

/datum/extension/mount/proc/on_mountee_move(atom/mover, oldloc, newloc)
