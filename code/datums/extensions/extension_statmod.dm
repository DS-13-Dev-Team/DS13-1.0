...
/datum/extension/proc/update_statmods()
	if(istype(holder, /mob))
		...
		CRASH("'update_statmods' called on atom that doesn't support statmods. Holder: [holder]")
...
/datum/extension/proc/register_statmod(var/modtype)
	...
	if(istype(holder, /mob))
		...
		CRASH("'register_statmod' called on atom that doesn't support statmods. Holder: [holder]")
...
/datum/extension/proc/unregister_statmod(var/modtype)
	...
	if(istype(holder, /mob))
		...
		CRASH("'unregister_statmod' called on atom that doesn't support statmods. Holder: [holder]")
