...
/datum/extension/appearance/New(var/holder)
	var/decl/appearance_manager/appearance_handler = appearance_manager.get_appearance_handler(appearance_handler_type)
	if(!appearance_handler)
		CRASH("Unable to acquire the [appearance_handler_type] appearance handler.")

	appearance_manager.RegisterSignal(holder, COMSIG_ITEM_EQUIPPED, item_equipment_proc)
	appearance_manager.RegisterSignal(holder, COMSIG_ITEM_UNEQUIPPED, item_removal_proc)
	.=..()
