...
/datum/component/fov_handler/Initialize(fov_type = FOV_180_DEGREES)
	...
	blocker_mask = new
	visual_shadow = new
	visual_shadow.alpha = parent_client?.prefs.fov_alpha
...
/datum/component/fov_handler/RegisterWithParent()
	...
	RegisterSignal(parent, COMSIG_MOB_ON_HUD_SHOWN, .proc/update_mask)
	RegisterSignal(parent, COMSIG_MOB_VIEW_CHANGED, .proc/update_fov_size)
	RegisterSignal(parent, COMSIG_MOB_RESET_VIEW, .proc/update_mask)
	RegisterSignal(parent, COMSIG_MOB_LOGOUT, .proc/mob_logout)

/datum/component/fov_handler/UnregisterFromParent()
	. = ..()
	UnregisterSignal(parent, list(COMSIG_MOB_VIEW_CHANGED, COMSIG_MOB_ON_HUD_SHOWN, COMSIG_MOB_RESET_VIEW, COMSIG_ATOM_DIR_CHANGE, COMSIG_LIVING_DEATH, COMSIG_LIVING_REVIVE, COMSIG_MOB_LOGOUT))
