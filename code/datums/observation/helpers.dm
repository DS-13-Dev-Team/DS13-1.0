/atom/movable/proc/recursive_move(var/atom/movable/am, old_loc, new_loc)
	GLOB.moved_event.raise_event(src, old_loc, new_loc)

/atom/movable/proc/move_to_turf(var/atom/movable/am, old_loc, new_loc)
	var/turf/T = get_turf(new_loc)
	if(T && T != loc)
		forceMove(T)

// Similar to above but we also follow into nullspace
/atom/movable/proc/move_to_turf_or_null(var/atom/movable/am, old_loc, new_loc)
	var/turf/T = get_turf(new_loc)
	if(T != loc)
		forceMove(T)

/atom/proc/recursive_dir_set(var/atom/a, old_dir, new_dir)
	if (new_dir != old_dir)
		set_dir(new_dir)
...
/proc/register_all_movement(var/event_source, listener)
...
/proc/unregister_all_movement(var/event_source, listener)
