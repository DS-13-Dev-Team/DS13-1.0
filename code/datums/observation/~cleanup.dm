...
/decl/observ/register(var/datum/event_source, datum/listener, proc_call)
	. = ..()
	if(.)
		GLOB.event_sources_count[event_source] += 1
		GLOB.event_listen_count[listener] += 1

/decl/observ/unregister(var/datum/event_source, datum/listener, proc_call)
	. = ..()
	if(.)
		GLOB.event_sources_count[event_source] -= 1
		GLOB.event_listen_count[listener] -= 1
...
/decl/observ/register_global(var/datum/listener, proc_call)
	. = ..()
	if(.)
		GLOB.global_listen_count[listener] += 1

/decl/observ/unregister_global(var/datum/listener, proc_call)
	. = ..()
	if(.)
		GLOB.global_listen_count[listener] -= 1
