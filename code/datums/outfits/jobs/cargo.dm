/decl/hierarchy/outfit/job/cargo
	hierarchy_type = /decl/hierarchy/outfit/job/cargo
	uniform = /obj/item/clothing/under/deadspace/cargo
	l_ear = /obj/item/radio/headset/headset_cargo
	glasses = /obj/item/clothing/glasses/sunglasses
	shoes = /obj/item/clothing/shoes/dutyboots

/decl/hierarchy/outfit/job/cargo/so
	name = OUTFIT_JOB_NAME("Supply Officer")
	l_ear = /obj/item/radio/headset/heads/so
	l_hand = /obj/item/clipboard
	id_type = /obj/item/card/id/holo/cargo/supply_officer
	pda_type = /obj/item/modular_computer/pda/heads/so

/decl/hierarchy/outfit/job/cargo/serviceman
	name = OUTFIT_JOB_NAME("Cargo Serviceman")
	id_type = /obj/item/card/id/holo/cargo
	pda_type = /obj/item/modular_computer/pda/cargo

/decl/hierarchy/outfit/job/cargo/salvage
	name = OUTFIT_JOB_NAME("Salvage Technician")
	id_type = /obj/item/card/id/holo/cargo/salvage_tech
	pda_type = /obj/item/modular_computer/pda/cargo