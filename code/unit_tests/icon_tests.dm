...
/datum/unit_test/icon_test/robots_shall_have_eyes_for_each_state/start_test()
	...
	for(var/icon_state in valid_states)
		...
		if(!(eye_icon_state in valid_states))
			log_test("Eye icon state [eye_icon_state] is missing.")
			missing_states++
	...
	if(missing_states)
		...
		if(difference.len)
			log_test("[ascii_yellow]---  DEBUG  --- ICON STATES AT START: " + jointext(original_valid_states, ",") + "[ascii_reset]")
			log_test("[ascii_yellow]---  DEBUG  --- ICON STATES AT END: "   + jointext(valid_states, ",") + "[ascii_reset]")
			log_test("[ascii_yellow]---  DEBUG  --- UNIQUE TO EACH LIST: " + jointext(difference, ",") + "[ascii_reset]")
	else
