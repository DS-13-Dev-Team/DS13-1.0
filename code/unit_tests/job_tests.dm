...
/datum/unit_test/jobs_shall_have_a_valid_outfit_type/start_test()
	...
	for (var/datum/job/occupation as anything in SSjobs.occupations)
...
/datum/unit_test/jobs_shall_have_a_HUD_icon/start_test()
	...
	var/job_huds = icon_states('icons/mob/hud.dmi')
	...
	if(failed_sanity_checks || failed_jobs)
		fail("'icons/mob/hud.dmi' - [failed_sanity_checks] failed sanity check\s, [failed_jobs] job\s with missing HUD icon.")
	else
