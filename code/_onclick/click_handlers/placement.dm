...
/datum/click_handler/placement/New(mob/user, datum/callback/C)
	if (C)
		call_on_place = C
...
/datum/click_handler/placement/OnLeftClick(atom/A, params)
	PLACEMENT_SAFETY
...
/datum/click_handler/placement/OnRightClick(atom/A, params)
	PLACEMENT_SAFETY
	stop_placement()


//Shiftclick: Spawn the item, but don't finish placement, so the user can keep placing more copies
/datum/click_handler/placement/OnShiftClick(atom/A, params)
	PLACEMENT_SAFETY
	//Check if we're allowed to place where we've clicked
	var/turf/site = get_turf_at_mouse(params, user.client)
	var/message = placement_blocked(site)
...
/datum/click_handler/placement/proc/placement_blocked(turf/candidate)
	PLACEMENT_SAFETY
...
//Preview Handling
//------------------------------------
/datum/click_handler/placement/proc/generate_preview_image(path)
	if (!preview)
...
/datum/click_handler/placement/proc/set_preview_color(placeable = FALSE)
	preview.alpha = 128
	if (placeable)
...
//Spawning
//----------------
/datum/click_handler/placement/proc/spawn_result(turf/site)
...
/proc/create_placement_handler(mob/_user, _result, _handler_type = /datum/click_handler/placement, snap = FALSE, datum/callback/C)
	if (!istype(_user))
		return
...
/datum/click_handler/placement/necromorph/placement_blocked(turf/candidate)
	.=..()
	if (!.)
		if (!biomass_source)
...
/datum/click_handler/placement/necromorph/spawn_result(turf/site)
...
/proc/create_necromorph_placement_handler(mob/_user, _result, _handler_type = /datum/click_handler/placement/necromorph, snap = TRUE, atom/biomass_source, name, biomass_cost, require_corruption)
	if (!istype(_user))
		return
