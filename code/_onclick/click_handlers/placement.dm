...
/datum/click_handler/placement/New(var/mob/user, datum/callback/C)
	if (C)
		call_on_place = C
...
/datum/click_handler/placement/OnLeftClick(var/atom/A, params)
	PLACEMENT_SAFETY
...
/datum/click_handler/placement/OnRightClick(var/atom/A, params)
	PLACEMENT_SAFETY
	stop_placement()


//Shiftclick: Spawn the item, but don't finish placement, so the user can keep placing more copies
/datum/click_handler/placement/OnShiftClick(var/atom/A, params)
	PLACEMENT_SAFETY
	//Check if we're allowed to place where we've clicked
	var/turf/site = get_turf_at_mouse(params, user.client)
	var/message = placement_blocked(site)
...
/proc/create_placement_handler(var/mob/_user, _result, _handler_type = /datum/click_handler/placement, snap = FALSE, datum/callback/C)
	if (!istype(_user))
		return
...
/proc/create_necromorph_placement_handler(var/mob/_user, _result, _handler_type = /datum/click_handler/placement/necromorph, snap = TRUE, atom/biomass_source, name, biomass_cost, require_corruption)
	if (!istype(_user))
		return
